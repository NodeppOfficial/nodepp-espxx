...
public:
   ...
   ~tcp_t() noexcept { if( obj.count() > 1 ){ return; } free(); }
    ...
    tcp_t( agent_t* opt=nullptr ) noexcept : obj( new NODE() )
...
namespace tcp {
    ...
    inline tcp_t server( agent_t* opt=nullptr ){ auto skt = tcp_t( opt ); return skt; }

    inline tcp_t client( agent_t* opt=nullptr ){ auto skt = tcp_t( opt ); return skt; }
